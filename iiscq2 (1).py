...
Original file is located at
    https://colab.research.google.com/drive/1gWzKvPSjO62eGiadTg3wzZeqqbdSlwus
"""
...
df = pd.read_csv(
    '/content/drive/MyDrive/combined_trajectories.csv', nrows=700000)
df.tail()
...
def calculate_user_distance(trajectories):
    ...
    for traj_df in trajectories:
        ...
        for i in range(1, len(traj_df)):
            lon1, lat1, alt1 = traj_df.iloc[i -
                                            1][['longitude', 'latitude', 'altitude']]
            lon2, lat2, alt2 = traj_df.iloc[i][[
...
def calculate_total_distance(df):
    ...
    user_trajectories = [create_user_trajectories(
        user_df) for _, user_df in user_groups]
...
# longitude and latitude coordinates of Beijing City
beijing_bbox = [115.42, 39.44, 117.50, 41.06]
...
geometry = [Point(xy)
            for xy in zip(gps_data['longitude'], gps_data['latitude'])]
crs = 'EPSG:4326'
...
hourly_density.plot(kind='bar', x='hour', y='count',
